...
public class Diary {
    ...
    private void addEntry() {
        ...
        if (!date.isEmpty() && !entry.isEmpty()) {
            ...
            showAlert("Error", "Date and entry cannot be empty.");
        }
    }

    private void addDiaryEntry(Connection conn, String date, String entry) throws SQLException {
        String sql = "INSERT INTO diary_entries (date, entry) VALUES (?, ?)";
    ...
    private void updateEntry() {
        ...
        if (selectedIndex != -1) {
            ...
            try (Connection conn = DriverManager.getConnection(JDBC_URL, DB_USER, DB_PASSWORD)) {
                updateDiaryEntry(conn, entryId, newEntry);
                displayDiaryEntries(conn);
                entryTextArea.clear();
            } catch (SQLException e) {
                e.printStackTrace();
    ...
    private void updateDiaryEntry(Connection conn, int entryId, String newEntry) throws SQLException {
        String sql = "UPDATE diary_entries SET entry = ? WHERE id = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    private void displayDiaryEntries(Connection conn) throws SQLException {
        ...
        try (Statement stmt = conn.createStatement();
            ...
            while (rs.next()) {
                ...
                entryListView.getItems().add(date + ": " + entry);
                entryIds.add(id);
            }
        }
    }
